...
RUN if [ "$TARGETPLATFORM" != "linux/amd64" && "$TARGETPLATFORM" != "linux/arm64" ]; then \
    echo 'TARGETPLATFORM must either be linux/amd64 or linux/arm64. Exiting.'; \
    exit 1; \
    fi
...
RUN if [ "$TARGETPLATFORM" = "linux/arm64" ]; then \
    set -eux && \
    curl --create-dirs -Lo /usr/lib/libfdb_c.so "${FDB_WEBSITE}/${FDB_VERSION}/libfdb_c.aarch64.so" && \
	dpkg -x foundationdb-clients_${FDB_VERSION}-1_amd64.deb /tmp/fdbclients && \
...
RUN rm foundationdb-clients_${FDB_VERSION}-1_amd64.deb foundationdb-clients_${FDB_VERSION}-1_amd64.deb.sha256
...
# Build
RUN if [ "$TARGETPLATFORM" = "linux/amd64" ]; then \
    CGO_ENABLED=1 GOOS=linux GOARCH=amd64 GO111MODULE=on make manager plugin-go \
    ;fi

RUN if [ "$TARGETPLATFORM" = "linux/arm64" ]; then \
    CGO_ENABLED=1 GOOS=linux GOARCH=arm64 GO111MODULE=on make manager plugin-go \
    ;fi
...
RUN if [ "$TARGETPLATFORM" = "linux/arm64" ]; then \
    ...
    rpm2cpio foundationdb-clients-${FDB_VERSION}-1.el7.x86_64.rpm | cpio -idmv --directory=/tmp/fdbclients && \
    cp -r /tmp/fdbclients/usr/include/* /usr/include/ && \
    rm -rf /tmp/fdbclients /tmp/fdbclients.deb \
    ;fi
...
RUN rm foundationdb-clients-${FDB_VERSION}-1.el7.x86_64.rpm foundationdb-clients-${FDB_VERSION}-1.el7.x86_64.rpm.sha256

COPY --from=builder /etc/passwd /etc/passwd
COPY --from=builder /etc/group /etc/group
COPY --from=builder /usr/lib/libfdb_c.so /lib64
